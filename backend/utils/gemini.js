...
export const generateMonthlyInsight = async ({
  ...
  const breakdownText =
    expenseBreakdown.length > 0
      ? expenseBreakdown
          .map((c) => `- ${c.category}: ${currency} ${safeToFixed(c.amount)}`)
          .join("\n")
  ...
  const trendText =
    previousMonths.length > 0
      ? previousMonths
          .map(
            (m) =>
              `- ${m.month}: Income ${currency} ${safeToFixed(m.income)}, Expenses ${currency} ${safeToFixed(m.expenses)}`,
          )
          .join("\n")
...
Total Income (this month): ${currency} ${safeToFixed(totalIncome)}
Total Expenses (this month): ${currency} ${safeToFixed(totalExpenses)}
Savings Rate: ${safeToFixed(savingsRate, 1)}%
...
Budget: ${currency} ${safeToFixed(budgetAmount)}
Spent so far: ${currency} ${safeToFixed(spentAmount)} (${percentUsed}% used)
  ...
  const categoryText =
    topCategories.length > 0
      ? topCategories
          .map(
            (c) =>
              `- ${c.category}: ${currency} ${safeToFixed(c.amount)} across ${c.transactionCount} transactions`,
          )
          .join("\n")
...
Monthly Income (last 30 days): ${currency} ${safeToFixed(monthlyIncome)}
...
export const analyzeTransactionList = async ({
  ...
  const lines = transactions
    ...
    .map((t) => {
      ...
      const amt = safeToFixed(t.amount);
...
export const analyzeBudgetList = async ({ budgets, currency = "USD" }) => {
  const lines = budgets
    .map((b) => {
      ...
      return `Budget ID ${b.id} | Category: ${b.category_name} | Limit: ${currency} ${safeToFixed(total)} | Spent: ${currency} ${safeToFixed(spent)} (${pct}%)`;
    })
    .join("\n");
